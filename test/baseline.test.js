...
let bbmWidely = getCompatibleVersions()
  .filter(version =>
...
test('Widely available versions from baseline-browser-mapping appear in browserslist output', () => {
  is(bbmWidely.reduce((acc, version) => {
    if (acc === false) return false
    return (
      browserslistBaselineWidely
        .filter(browser =>
          browser.indexOf(nameMapping[version.browser]) !== -1
        )
        .find(browser =>
        (
...
let date30MonthsFromNow = new Date(new Date().setMonth(new Date().getMonth() + 30)).toISOString().slice(0, 10);
let browserslistBaselineNewly = browserslist('baseline newly available')
let browserslistBaselineWidelyByDate = browserslist(`baseline widely available on ${date30MonthsFromNow}`)
...
let baseline2020MinVersionsWithDownstream = [...baseline2020MinVersions,
  'samsung 14.0',
  'opera 73',
  'kaios 3.0-3.1',
]

let browserslistBaseline2020 = browserslist('baseline 2020 with downstream including kaios');

test('Selects proper core versions for baseline 2020', () => {
  is(baseline2020MinVersions.reduce((acc, curr) => {
    return acc === false ? false : browserslistBaseline2020.indexOf(curr) !== -1;
  }, true), true);
});

test('Selects proper downstream versions for baseline 2020', () => {
  is(baseline2020MinVersionsWithDownstream.reduce((acc, curr) => {
    return acc === false ? false : browserslistBaseline2020.indexOf(curr) !== -1;
  }, true), true);
});
