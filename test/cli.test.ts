...
async function arr (...args: any[]) {
  let stdout = await out(...args)
  return stdout.split('\n').filter(Boolean)
}

type Query = Parameters<typeof browserslist.coverage>[0]
type Area = Parameters<typeof browserslist.coverage>[1]

function coverage (query: Query, area?: Area) {
  let result = browserslist.coverage(query, area)
  return (Math.round(result * 100) / 100.0)
}

function ie8cov (area?: Area) {
